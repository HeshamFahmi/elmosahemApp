...
class _AddressDetailsFormState extends State<AddressDetailsForm> {
    ...
    return TextFormField(
      ...
      validator: (value) {
        if (pincodeFieldController.text.isEmpty) {
          ...
          return "المسموح ارقام فقط";
        } else if (pincodeFieldController.text.length != 6) {
          return "مسموح ب6 ارقام فقط";
        }
        return null;
      },
      autovalidateMode: AutovalidateMode.onUserInteraction,
  ...
  Widget buildLandmarkField() {
    return TextFormField(
      ...
      keyboardType: TextInputType.name,
      decoration: InputDecoration(
        hintText: "ادخل علامه مميزه",
        labelText: "علامه مميزه",
        floatingLabelBehavior: FloatingLabelBehavior.always,
      ),
      validator: (value) {
  ...
  Future<void> saveNewAddressButtonCallback() async {
    if (_formKey.currentState.validate()) {
      ...
      try {
        ...
        if (status == true) {
          snackbarMessage = "تم تسجيل العنوان بنجاح";
        } else {
          throw "تعذر الاضافه حاول مره اخري";
        }
  ...
  Future<void> saveEditedAddressButtonCallback() async {
    if (_formKey.currentState.validate()) {
      ...
      try {
        ...
        if (status == true) {
          snackbarMessage = "تم التعديل بنجاح";
        } else {
          throw "حدث خطأ ما حاول مره اخري";
        }
      } on FirebaseException catch (e) {
        Logger().w("Firebase Exception: $e");
        snackbarMessage = "حدث خطأ ما";
